...
The four paid solver trials ran sequentially on one macOS host/network using fresh contexts. The first invalid-token control overlapped the second 2Captcha workflow in a separate browser. Ordering, shared IP reputation, headed/headless executable differences and the small sample prevent a causal or statistical ranking. We did not configure proxy rotation, personal browser profiles or login cookies. The later proxyless services use their own solver infrastructure and may use human workers; their solving environment is not the local browser environment. The final headed audio trial checked the attempt-budget limitation; it was rate-limited, after which audio trials stopped.
