...
The old recipe produces HTTP 400 in this fixture, with no origin hit. That result is not a universal outcome for every target or upstream. Missing/wrong credentials produce HTTP 407 here; cURL exits 22 for plain HTTP and 56 for failed CONNECT. Requests exposes an HTTP 407 response for HTTP and raises ProxyError for HTTPS. Messages and OS error numbers may differ elsewhere.
