...
for i in $(seq 1 20); do python -c "import socket; socket.create_connection(('127.0.0.1', 8000), 1)" 2>/dev/null && break; sleep 0.5; done
...
{ echo "wget: $(wget --version | head -1)"; echo "curl: $(curl --version | head -1)"; echo "python: $(python --version 2>&1)"; } > expected_output/00_versions.txt
...
rec 07_error_no_body  "wget -nv -O- --post-data 'x=1' $U/error400; echo \"---\"; wget -nv -O response.json --post-data 'x=1' $U/error400; ls -l response.json"   "cd /tmp && rm -f response.json && wget -nv -O- --post-data 'x=1' $U/error400; echo '---'; wget -nv -O response.json --post-data 'x=1' $U/error400; ls -l response.json | awk '{print \$5, \$9}'"
...
rec 12_urlencode      "wget -qO- --post-data \"\$(python -c 'import urllib.parse; print(urllib.parse.urlencode({\"q\": \"hello world\", \"note\": \"a&b=c\"}))')\" $U/echo"   "wget -qO- --post-data \"\$(python -c 'import urllib.parse; print(urllib.parse.urlencode({\"q\": \"hello world\", \"note\": \"a&b=c\"}))')\" $U/echo"
...
rec 13c_save_without_keep "wget -qO- --save-cookies cookies.txt --post-data 'user=foo&password=bar' $U/login && grep -c session cookies.txt"   "cd /tmp && rm -f cookies.txt && wget -qO- --save-cookies cookies.txt --post-data 'user=foo&password=bar' $U/login && grep -c session cookies.txt"
...
rec 16_retry_default  "wget -nv -O- --tries=3 --post-data 'x=1' $U/flaky"                                    "wget -nv -O- --tries=3 --post-data 'x=1' $U/flaky"
rec 17_retry_on_503   "wget -nv -O- --tries=3 --retry-on-http-error=503 --waitretry=1 --post-data 'x=1' $U/flaky"   "wget -nv -O- --tries=3 --retry-on-http-error=503 --waitretry=1 --post-data 'x=1' $U/flaky"
...
sed -i.bak -E 's/"user_agent": "(Wget|curl)\/[^"]*"/"user_agent": "\1\/<version>"/; s/^  Server: .*/  Server: <server>/; s/^  Date: .*/  Date: <date>/; s/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} /<time> /; s/boundary=-+[A-Za-z0-9]+/boundary=<boundary>/; s/^-+[A-Za-z0-9]+\\r/--<boundary>\\r/; s/\\r\\n-+[A-Za-z0-9]+/\\r\\n--<boundary>/g' expected_output/*.txt; rm -f expected_output/*.bak
...
g 04 '"method": "PUT"'; g 05 '"method": "POST"'; g 06 "HTTP/1.1 200"; g 07 "ERROR 400"; g 08 "field user is required"
g 09_redirect_303 '"method": "GET"'; g 09_redirect_307 '"method": "POST"'; g 09_redirect_308 '"method": "POST"'; g 10 "exit="; g 12 "q=hello+world"
g 13_ "hello foo"; g 13b "Authentication Failed"; g 13c "^0"; g 14 '"authorization": "Basic'; g 15 '"authorization": "Basic'; g 17 '"method": "POST"'; g 18 "exit=4"; g 19 "multipart/form-data"
